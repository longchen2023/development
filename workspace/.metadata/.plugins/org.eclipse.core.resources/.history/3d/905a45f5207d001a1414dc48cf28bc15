...
public interface  IzakayaManageDao {
	...
	@Insert({ "insert into izakaya_info("
	 ...
	 int insertIzakayaInfo(@Param("id")String id,@Param("user_id")String user_id,@Param("name")String name,
			 ...
			 @Param("price")String price,@Param("opening_time")String opening_time,@Param("closing_time")String closing_time,@Param("holiday")String holiday,@Param("url")String url,
	 ...
	 */
	 ...
	 @Select("select * from izakaya_info "
	 		+ "where "
	 		+ "max_num_of_people >= #{reservationNums,jdbcType=NUMERIC} "
	 		+ "and price >= #{minAmount,jdbcType=NUMERIC} "
	 		+ "and price <= #{maxAmount,jdbcType=NUMERIC} "
	 		+ "and area = #{area,jdbcType=VARCHAR} "
	 		+ "and genre = #{genre,jdbcType=VARCHAR}")
	 List<IzakayaInfo> selectIzakayaListBySearchForm(@Param("reservationNums")String reservationNums,
			 @Param("minAmount")String minAmount,@Param("maxAmount")String maxAmount,@Param("area")String area,
	  ...
	  * @return 
	  */
	 @Delete("delete from izakaya_info where id = #{id,jdbcType=NUMERIC}")
	 int deleteIzakayaInfo(@Param("id")String id);
